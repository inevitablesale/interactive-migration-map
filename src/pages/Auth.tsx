...
import { DocumentSigningCheck } from "@/components/DocumentSigningCheck";
...
export default function Auth() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      if (session) {
        // Don't navigate directly, let DocumentSigningCheck handle the flow
        console.log("User is logged in, checking documents...");
      }
    };
    checkSession();
  }, [navigate]);
  ...
  const handleLinkedInAuth = async () => {
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          redirectTo: `${window.location.origin}/auth`, // Redirect back to auth to check documents
        },
      });
